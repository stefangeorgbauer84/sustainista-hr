...
import { Clock, Calendar, FileText, TrendingUp, AlertCircle } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          ...
          label="Diesen Monat"
          value={formatDuration(totalMinutesThisMonth)}
          sub="Arbeitszeit"
          color="blue"
        />
        <StatCard
          ...
          label="Urlaubstage"
          value={`${vacationLeft} / ${employee?.vacationDaysTotal ?? 25}`}
          sub="verbleibend"
          color="green"
        />
        <StatCard
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <QuickLink
          ...
          icon={<Clock className="h-5 w-5" strokeWidth={1.5} />}
          title="Zeiterfassung"
          desc="Arbeitszeit starten, stoppen oder manuell eintragen"
        />
        <QuickLink
          href="/dashboard/leave"
          icon={<Calendar className="h-5 w-5" strokeWidth={1.5} />}
          title="Urlaubsantrag stellen"
          desc="Urlaub oder Krankenstand eintragen"
        />
        <QuickLink
          href="/dashboard/documents"
          icon={<FileText className="h-5 w-5" strokeWidth={1.5} />}
          title="Meine Dokumente"
          desc="Lohnzettel und Verträge abrufen"
        />
      </div>
    </div>
  );
...
function QuickLink({ href, icon, title, desc }: { href: string; icon: React.ReactNode; title: string; desc: string }) {
  return (
    <Link
      href={href}
      className="flex items-start gap-4 rounded-xl border border-gray-200 bg-white p-4 transition hover:border-[#4F772D]/40 hover:shadow-sm"
    >
