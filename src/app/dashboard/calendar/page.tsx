    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">Übersicht über Abwesenheiten im Team</p>
        </div>
      ...
      {absentToday.length > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          ...
          <p className="text-sm text-amber-700">
            <strong>Heute abwesend:</strong>{" "}
            {absentToday.map(e => `${e.firstName} ${e.lastName}`).join(", ")}
          </p>
        </div>
      )}
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white">
            ...
            <tr className="border-b border-gray-100 bg-gray-50">
              ...
              {days.map(day => (
                <th key={day.toISOString()}
                  className={`px-0.5 py-3 text-center font-medium min-w-[30px] ${
                ...
                {days.map(day => {
                  ...
                  return (
                    <td key={day.toISOString()}
                      className={`px-0.5 py-1.5 text-center ${weekend ? "bg-gray-50/50" : today_ ? "bg-[#4F772D]/5" : ""}`}>
                      {leave && !weekend && (
                        <div
        ...
        <div className="border-t border-gray-100 px-4 py-3 flex items-center gap-5">
          {Object.entries(typeLegend).map(([key, label]) => (
            <div key={key} className="flex items-center gap-1.5">
              <div className={`flex h-4 w-4 items-center justify-center rounded border text-[8px] font-bold ${typeColors[key]}`}>
                {typeShort[key]}
              </div>
