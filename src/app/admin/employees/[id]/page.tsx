...
import type { Employee, TimeEntry, LeaveRequest, Document } from "@/types";
...
export default function EmployeeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: employee } = useQuery<Employee>({
    ...
    queryFn: async () => {
      const doc = await databases.getDocument(DB_ID, COLLECTIONS.EMPLOYEES, id);
  ...
  const totalMins = timeEntries.filter(e => e.status !== "running").reduce((s, e) => s + calcWorkedMinutes(e), 0);
  const overtime = totalMins - 160 * 60;
  const sickDaysThisYear = leaves.filter(l =>
    l.type === "sick" && l.status === "approved" && l.startDate.startsWith(String(now.getFullYear()))
  ).reduce((s, l) => s + l.days, 0);

  // EFZG Entgeltfortzahlung: 6 Wochen (30 Arbeitstage) voll, dann gestaffelt
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="mt-1.5 flex flex-wrap gap-2 text-xs text-gray-400">
                {employee.email && <span className="flex items-center gap-1"><Mail className="h-3 w-3" strokeWidth={1.5} />{employee.email}</span>}
                {employee.phone && <span className="flex items-center gap-1"><Phone className="h-3 w-3" strokeWidth={1.5} />{employee.phone}</span>}
                {employee.address && <span className="flex items-center gap-1"><MapPin className="h-3 w-3" strokeWidth={1.5} />{employee.address}</span>}
              </div>
            </div>
          </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Stat label="Eintrittsdatum" value={format(parseISO(employee.startDate), "d. MMM yyyy", { locale: de })} icon={<Building className="h-4 w-4" strokeWidth={1.5} />} />
          <Stat label="Urlaubstage" value={`${employee.vacationDaysTotal - employee.vacationDaysUsed}/${employee.vacationDaysTotal}`} icon={<Calendar className="h-4 w-4" strokeWidth={1.5} />} />
        ...
        {employee.bankAccount && (
          <div className="mt-4 flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-500">
            ...
            IBAN: <span className="font-mono font-medium text-gray-700">{employee.bankAccount}</span>
          </div>
        )}
      </div>
          ...
          <div className="divide-y divide-gray-50 max-h-64 overflow-y-auto">
            {timeEntries.length === 0 ? (
              ...
              <div key={e.$id} className="flex items-center justify-between px-5 py-2.5">
                <div>
                  <p className="text-xs font-medium text-gray-900">{format(parseISO(e.date), "EEE d. MMM", { locale: de })}</p>
                  <p className="text-[10px] text-gray-400">{e.startTime} – {e.endTime ?? "läuft"}</p>
                </div>
                <p className="text-xs font-medium text-gray-700">{e.endTime ? formatDuration(calcWorkedMinutes(e)) : "—"}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="divide-y divide-gray-50 max-h-64 overflow-y-auto">
            {leaves.length === 0 ? (
              ...
              <div key={l.$id} className="flex items-center justify-between px-5 py-2.5">
                <div>
                  <p className="text-xs font-medium text-gray-900">{typeLabels[l.type]} · {l.days} Tage</p>
                  <p className="text-[10px] text-gray-400">
                    {format(parseISO(l.startDate), "d. MMM", { locale: de })} – {format(parseISO(l.endDate), "d. MMM yyyy", { locale: de })}
                  </p>
                </div>
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${statusColors[l.status]}`}>
                  {l.status === "approved" ? "Genehmigt" : l.status === "pending" ? "Offen" : "Abgelehnt"}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="divide-y divide-gray-50">
          {docs.length === 0 ? (
            ...
            <div key={doc.$id} className="flex items-center justify-between px-5 py-3">
              ...
              <a
                href={`https://cloud.appwrite.io/v1/storage/buckets/documents/files/${doc.fileId}/download?project=6a2567ad0021c84890d1`}
                target="_blank" rel="noreferrer"
                className="text-xs text-[#4F772D] hover:underline"
              >
                Download
              </a>
            </div>
